...
S1 <- inla.surv(time = survDat$deathTimes, event = survDat$Event)
JMinla <- joint(formLong = list(Y1 ~ time + ctsX + binX + (1 + time|Id),
                                Y2 ~ time + ctsX + binX + (1 + time|Id),
                                Y3 ~ time + ctsX + binX + (1|Id)),
                formSurv = S1 ~ -1, dataSurv = survDat, dataLong = longDat,
                basRisk = "rw2", id = "Id", corLong=TRUE, timeVar = "time",
                family = c("gaussian", "poisson", "binomial"),
                assoc = c("CV", "CV", "CV"), control=list(int.strategy="eb",
                priorFixed=list(mean=0, prec=0.16, mean.intercept=0, prec.intercept=0.16),
                priorAssoc=list(mean=0, prec=0.16)))
summary(JMinla)

# model fit with JMbayes2
if(F){
  library(JMbayes2) # MCMC
  CoxFit <- try(coxph(Surv(deathTimes, Event) ~ 1, data = survDat))
  fm1 <- try(lme(Y1 ~ time + ctsX + binX, data = longDat,
                 random = ~ time | Id))
  fm2 <- try(mixed_model(Y2 ~ time + ctsX + binX, data = longDat,
                         random = ~ time | Id, family = poisson(link = "log")))
  fm3 <- try(mixed_model(Y3 ~ time + ctsX + binX, data = longDat,
                         random = ~ 1 | Id, family = binomial(link = "logit")))
  JMpr = list(mean_alphas=list(0,0,0), Tau_alphas = list(matrix(0.16), matrix(0.16), matrix(0.16)))
  JMB <- try(jm(CoxFit, list(fm1, fm2, fm3), time_var = "time", priors=JMpr,
                n_iter = 3500L, n_burnin = 500L))
}
...
# Model fit with INLA (i.e., without INLAjoint)
if(F){
